	...
	if(!$bbcodeoff && $allowbbcode) {
		...
		if($parsetype != 1 && strpos($msglower, '[/hide]') !== FALSE && $pid) {
			...
			if(strpos($msglower, '[hide]') !== FALSE) {
				...
				$rateexist = DB::result_first("SELECT pid FROM ".DB::table('forum_ratelog')." WHERE pid='{$pid}' AND uid='$_G[uid]' ");
				if($authorreplyexist || $rateexist) {
					$message = preg_replace("/\[hide\]\s*(.*?)\s*\[\/hide\]/is", tpl_hide_reply(), $message);
				} else {
					$message = preg_replace("/\[hide\](.*?)\[\/hide\]/is", tpl_hide_reply_hidden($pid), $message);
